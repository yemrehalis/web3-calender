...
describe("Calend3", function () {
  ...
  it("Should fail if non-owner rate", async function () {
    ...
    await expect(
      contract.connect(addr1).setRate(500)).to.be.revertedWith('Only the owner can set the rate');
  ...
  it("Should add two appointments", async function () {
    ...
    const tx2 = await contract.connect(addr1).createAppointment("Meeting with the Emre De Marco", 1644143400, 1644150600, { value: ethers.utils.parseEther("2") });
    await tx2.wait();

    const tx3 = await contract.connect(addr2).createAppointment("Breakfast at Seda's", 1644154200, 1644159600, { value: ethers.utils.parseEther("1.5") });
    await tx3.wait();
